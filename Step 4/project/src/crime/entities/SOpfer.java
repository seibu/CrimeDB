...
public class SOpfer extends Entity {
    ...
    public SOpfer(ResultSet rs) throws SQLException {
    ...
    public static ObservableList<SOpfer> buildData(Connection c, TableView tableView, String value) {
        try {
            String query = "SELECT DISTINCT p.* FROM Personen p" +
                    " INNER JOIN Opfer v ON v.PersNr = p.PersNr" +
    ...
    public static ObservableList<SOpfer> buildDataInner(Connection c, ResultSet rs, TableView tableView) throws SQLException{
        ObservableList<SOpfer> data = FXCollections.observableArrayList();
